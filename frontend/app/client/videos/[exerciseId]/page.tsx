...
import { getClientExerciseVideos, type ClientExerciseVideosResponse } from "@/lib/api";

export default function ClientExerciseVideosPage() {
  const router = useRouter();
  const params = useParams<{ exerciseId: string }>();
  const exerciseId = useMemo(() => Number(params.exerciseId), [params.exerciseId]);
  const [exercise, setExercise] = useState<ClientExerciseVideosResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await getClientExerciseVideos(token, exerciseId);
        setExercise(data);
      } catch (err) {
