...
clientRouter.get("/days/:dayNumber", async (req, res) => {
  const userId = req.user!.userId;
...
clientRouter.patch("/days/:dayNumber/items/:itemId", async (req, res) => {
  const userId = req.user!.userId;
  const dayNumber = Number(req.params.dayNumber);
  ...
  const { clientSet1Kg, clientSet2Kg, clientSet3Kg, clientSet4Kg } = req.body as {
  ...
  const invalid = [clientSet1Kg, clientSet2Kg, clientSet3Kg, clientSet4Kg].some(
    (v: unknown) => {
...
clientRouter.get("/videos/exercises", async (req, res) => {
  const userId = req.user!.userId;
  const clientProfile = await prisma.clientProfile.findUnique({
    where: { clientUserId: userId },
...
clientRouter.get("/videos/exercises/:exerciseId", async (req, res) => {
  const userId = req.user!.userId;
